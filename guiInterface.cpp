	...
	while(1) {
		...
		while(1) {
			...
			if(atoi(sep[2].c_str())==0)
			 ...
			 if(atoi(sep[2].c_str())==1)
				{
					string message = "gnome-terminal -e 'sh -c \"./dchat "+name+"\"'";
					system(message.c_str());
				}
...
void *android_interface_receiver_handler(void *)
{
	bzero(receive_data, 1024);
	while(1){
	bytes_recieved = recv(interface_newSocket,receive_data,1024,0);                     
    
      
	cout<<"Bytes received from phone = "<<receive_data<<endl;
	if(bytes_recieved>0)
	{
...
void *android_interface_sender_handler(void *)
{
	...
	cout<<"Received from masked client"<<endl;
